...
class BandedMatrix:
    ...
    def band_shift(self):
        ...
        return BandedMatrix(
            torch.cat(
                [self.data[:, :, 1:], torch.zeros(batch, n, 1).fill_(self.fill)], 2
            ),
            self.lu - 1,
            self.ld + 1,
            self.fill,
    ...
    def band_unshift(self):
        ...
        return BandedMatrix(
            torch.cat(
                [torch.zeros(batch, n, 1).fill_(self.fill), self.data[:, :, :-1]], 2
            ),
            self.lu - 1,
            self.ld + 1,
            self.fill,
    ...
    def col_shift(self):
        ...
        return BandedMatrix(
            torch.cat(
                [self.data[:, 1:, :], torch.zeros(batch, 1, off).fill_(self.fill)], 1
            ),
            self.lu - 1,
            self.ld + 1,
            self.fill,
    ...
    def col_unshift(self):
        ...
        return BandedMatrix(
            torch.cat(
                [torch.zeros(batch, 1, off).fill_(self.fill), self.data[:, :-1, :]], 1
            ),
            self.lu + 1,
            self.ld - 1,
            self.fill,
