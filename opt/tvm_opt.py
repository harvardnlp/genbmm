...
logsummulouter_pytorch = to_pytorch_func(mod)
...
print(logsummulouter_pytorch(X, Y1, Y2, out, M))

out2 = ((Y1[:, :, None, :] * Y2[:, None, :, :]).view(1, 1, S3, S2) + X.view(1, S1, 1, S2)).logsumexp(-1)
print(torch.isclose(out2.transpose(1, 2), out))
#exit()
...
X = torch.rand(1, S1, S2).cuda()
Y = torch.rand(1, S3, S2).cuda()
#Y.requires_grad_(True)
out = torch.rand(1, S3, S1).cuda()
M = torch.rand(1, S3, S1).cuda()

logsum_pytorch = to_pytorch_func(mod)
logsum_pytorch(X, Y, out, M)
exit()
